# server.py (Final version with correct RAG chain input handling)
...
try:
    ...
    print(f"Could not load the local transcription model. Error: {e}")
    exit()
...
def transcribe_video(state: GraphState):
    ...
    try:
        ...
        print(f"ERROR: yt-dlp failed to download the audio.")
        print(f"Stderr from yt-dlp: {e.stderr}")
        raise e
    except Exception as e:
...
def write_blog_rag(state: GraphState):
    ...
    # THE FIX: This chain correctly plucks the 'title' from the input to pass
    # to the retriever, then passes the original input plus the new 'context'
    # to the prompt.
    rag_chain = (
    ...
    draft_blog = rag_chain.invoke({"title": title})
...
# --- 6. SERVE WITH LANGSERVE ---
api = FastAPI(
    title="Advanced YouTube to Blog Post Agent",
    version="1.0",
    description="An API that uses a multi-agent graph with RAG and self-correction to generate a blog post from a YouTube URL.",
)

class BloggerInput(BaseModel):
    youtube_url: str

runnable_with_types = app.with_types(input_type=BloggerInput)

add_routes(
    api,
    runnable_with_types,
    path="/youtube-blogger",
